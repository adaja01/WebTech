    ...
    db.all("SELECT * FROM games WHERE is_upcoming = 0", [], (err, games) => {
      ...
      const leaderboard = teams.map((team) => {
        ...
        const homeWins = homeGames.filter(
          (g) => g.home_score > g.away_score,
        ).length;
        const awayWins = awayGames.filter(
          (g) => g.away_score > g.home_score,
        ).length;
        ...
        const mapsWon =
          homeGames.reduce((sum, g) => sum + g.home_score, 0) +
          awayGames.reduce((sum, g) => sum + g.away_score, 0);
        const mapsLost =
          homeGames.reduce((sum, g) => sum + g.away_score, 0) +
...
baseRouter.post("/api/register", async (req, res) => {
  ...
  db.get("SELECT * FROM users WHERE email = ?", [email], async (err, row) => {
    ...
    db.run(
      ...
      [first_name, last_name, email, hashedPass, favorite_team_id],
      (insertErr) => {
        if (insertErr) {
...
baseRouter.put("/api/profile", requireLogin, async (req, res) => {
  ...
  const updateQuery = updatedPassword
  ...
  const updateParams = updatedPassword
    ? [
        updatedFirstName,
        updatedLastName,
        updatedEmail,
